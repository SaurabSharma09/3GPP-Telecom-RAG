...
class HybridRetriever:
    ...
    def __init__(
        ...
        with open(
            ...
            self.vector_chunks = pickle.load(
                file
            )
    ...
    def section_score(
        ...
        if query_type == "ROLE":
            ...
            if any(
                term in title
                for term in interface_title_terms
            ):
            ...
            if any(
                term in title
                for term in procedure_title_terms
            ):
            ...
            if any(
                term in title
                for term in comparison_title_terms
            ):
            ...
            if any(
                term in title
                for term in definition_title_terms
            ):
    ...
    @staticmethod
    def exact_interface_entity_score(
        text: str,
        section_title: str,
        entities
    ):
    ...
    def calculate_relevance_features(
        self,
        result,
        query,
        query_type,
        ...
        if query_type == "INTERFACE":
            ...
            exact_interface = (
                self.exact_interface_entity_score(
                    text=text,
                    section_title=section_title,
                    entities=entities
                )
    ...
    def apply_query_intent(
        ...
        CONCEPT_WEIGHT = 0.020

        SECTION_WEIGHT = 0.025

        # Stronger than generic interface relevance.
        INTERFACE_WEIGHT = 0.040

        # Strongest special signal for interface entities.
        EXACT_INTERFACE_WEIGHT = 0.060

        for result in results:

            features = (
                self.calculate_relevance_features(
                    result,
                    query,
                    query_type,
                    entities
            ...
            intent_score = features[
                "intent_score"
            ]

            concept_score = features[
                "concept_score"
            ]

            section_score = features[
                "section_score"
            ]

            interface_score = features[
                "interface_score"
            ]

            exact_interface_score = features[
                "exact_interface_score"
            ]
...
def print_results(
    ...
    if results:
        ...
        print(
            f"Query Type: "
            f"{results[0].get('query_type')}"
        )

        print(
            f"Entities: "
            f"{results[0].get('query_entities')}"
        )
